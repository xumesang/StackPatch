...
uint8_t patch[USART_REC_LEN];  //__attribute__ ((at(0x20008000)));
/*  接收状态
 *  bit15，      接收完成标志
 *  bit14，      接收到0x0d
 *  bit13~0，    接收到的有效字节数目
*/
//void patchfunc(void) __attribute__((section(".ARM.__at_0x20008000")));
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART_UX)               /* 如果是串口1 */
    {
        ...
        {
            patch[g_usart_rx_cnt] = g_rx_buffer[0];
            g_usart_rx_cnt++;
        }
