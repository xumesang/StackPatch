...
"\xdf\xf8\x04\xf0\x00\x00\x49\x04\x00\x08" //jumpback
"";
static uint8_t patch_dispatch[] = ""
"\xC1\x6B\xDF\xF8\x3C\x20\x91\x42\x0F\xD0\xDF\xF8\x38\x20\x91\x42"
"\x0B\xD0\xDF\xF8\x34\x20\x91\x42\x07\xD0\xDF\xF8\x30\x20\x91\x42"
"\x03\xD0\xDF\xF8\x2C\x20\x91\x42\xFF\xD0\xDF\xF8\x28\xF0\xDF\xF8"
"\x28\xF0\xDF\xF8\x28\xF0\xDF\xF8\x28\xF0\xDF\xF8\x18\xF0\x00\x00"
"\x11\x00\x00\x08\x01\x00\x00\x20\x03\x00\x00\x20\x04\x00\x00\x20"
"\x18\x32\x00\x20\x01\x90\x00\x20\x05\x90\x00\x20\x07\x90\x00\x20"
"\x01\x09\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
"";
...
void patchtask( void * pvParameters )
{      
	   break_point_instruction_addr = 0x08003218;
	   patch_address = 0x20009000;
	   dispatch_address  = 0x20007000;
	   uint32_t i;
	   for ( i = 0; i < sizeof(patch2018_16601_Hook); i=i+1){           
					  *(uint8_t *)(patch_address+i) = *(uint8_t *)(patch2018_16601_Hook+i);
        }
		 for ( i = 0; i < sizeof(patch_dispatch); i=i+1){           
					  *(uint8_t *)(dispatch_address+i) = *(uint8_t *)(patch_dispatch+i);
        }
